...
import { AnalysisResult, PartnerVibe } from '../types/index.js';
...
// Initialize SiliconFlow client (only in non-mock mode)
...
/**
 * Mock analysis data (based on vibe and score)
 */
...
/**
 * Generate destiny analysis using DeepSeek
...
export const generateDestinyAnalysis = async (
  ...
  // Mock mode: return preset data
  if (USE_MOCK) {
    console.log('[MOCK] Generating mock destiny analysis:', vibe, score);
...
请生成一份个性化的缘分解读，以 JSON 格式返回。`;
  ...
  try {
    ...
    try {
      ...
      // Fall back to mock mode on parse failure
      return generateMockAnalysis(vibe, score);
    }
  } catch (error) {
    console.error('DeepSeek API call failed:', error);
    // Fall back to mock mode on API error
    console.log('[MOCK] API failed, using mock data');
    return generateMockAnalysis(vibe, score);
  }
};
